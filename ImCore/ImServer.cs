...
class ImServer : ImClient
{
    ...
    ConcurrentDictionary<string, ConcurrentDictionary<string, ImServerClient>> _clients = new ConcurrentDictionary<string, ConcurrentDictionary<string, ImServerClient>>();
    ...
    void RedisSubScribleMessage(CSRedis.CSRedisClient.SubscribeMessageEventArgs e)
    {
        try
        {
            var data = JsonConvert.DeserializeObject<(string senderClientId, string[] receiveClientId, string content, bool receipt)>(e.Body);
            ...
            foreach (var clientId in data.receiveClientId)
            {
                if (_clients.TryGetValue(clientId, out var wslist) == false)
                {
                    ...
                    if (data.senderClientId != string.Empty && clientId != data.senderClientId && data.receipt)
