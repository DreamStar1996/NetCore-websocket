...
namespace Web.Redis
{
    ...
    public class MyRedisHelper
    {
        ...
        #region 构造函数
        public static MyRedisHelper Instance(int dbNum = 0)
        {
            return new MyRedisHelper(dbNum, null);
        }
        public static MyRedisHelper Instance(string readWriteHosts, int dbNum = 0)
        {
            return new MyRedisHelper(dbNum, readWriteHosts);
        }

        private MyRedisHelper(int dbNum, string readWriteHosts)
        {
            DbNum = dbNum;
            _conn =
        ...
        public async Task<bool> SetAddAsync<T>(string key, T value, double score)
        {
            key = AddSysCustomKey(key);
        ...
        {
            key = AddSysCustomKey(key);
            return Do(redis => redis.SetContainsAsync(key, ConvertJson(value)));
        }

        #endregion 异步方法

        #endregion SortedSet 集合
        ...
        public List<ElementScore<T>> SortedSetRangeByRankWithScores<T>(string key)
        {
            key = AddSysCustomKey(key);
            return Do(redis =>
            {
                var values = redis.SortedSetRangeByRankWithScores(key);
                return ConvetList<T>(values);
            });
        }
        /// <summary>
        ...
        {
            key = AddSysCustomKey(key);
            return Do(redis => redis.SortedSetRank(key, ConvertJson(value))) != null;
        }
        #endregion 同步方法
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public bool SortedSetContainsAsync<T>(string key, T value)
        {
            key = AddSysCustomKey(key);
            return Do(redis => redis.SortedSetRankAsync(key, ConvertJson(value))) != null;
        }
        ...
        private List<ElementScore<T>> ConvetList<T>(SortedSetEntry[] values)
        {
            List<ElementScore<T>> result = new List<ElementScore<T>>();
            foreach (var item in values)
            {
                var model = ConvertObj<T>(item.Element);
                result.Add(new ElementScore<T>() { Element = model, Score = item.Score });
            }
            return result;
        }
